...
import 'package:my_app/student/student_approval_detail_page.dart';
...
class StudentPendingApprovalsPage extends StatefulWidget {
  final String studentId;
  final String courseId;
  final String courseName;

  const StudentPendingApprovalsPage({
    super.key,
  ...
  @override
  State<StudentPendingApprovalsPage> createState() =>
      _StudentPendingApprovalsPageState();
}

class _StudentPendingApprovalsPageState
    extends State<StudentPendingApprovalsPage> {
  ...
  Future<List<ApprovalItem>> _loadList() async {
    final res = await http.get(
      Uri.parse(
        '$apiBase/file_manage.php?type=student_leave_list'
  ...
  (String, Color) _statusLabel(ApprovalStatus s) {
    switch (s) {
      ...
      case ApprovalStatus.pending:
        return ('รออนุมัติ', const Color(0xFFF59E0B));
    }
  }
                ...
                return TextBox(
                  ...
                  onTap: () {
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (_) =>
                            StudentApprovalDetailPage(approvalId: it.id),
                      ),
                    ).then((_) => _refresh());
                  },
                );
